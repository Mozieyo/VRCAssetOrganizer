...
def ensure_single_instance() -> QSharedMemory | None:
    ...
    mutex_name = "Global\\BoothOrganizerSingleInstance"
    mutex = kernel32.CreateMutexW(None, False, mutex_name)
    if mutex == 0:
        # Mutex creation failed entirely — fall back to shared memory
        shared = QSharedMemory("BoothOrganizerSingleInstance")
        if shared.create(1):
            return shared
        if shared.attach():
            shared.detach()
...
def get_app_data_dir() -> Path:
    base = Path.home() / "AppData" / "Local" / "BoothOrganizer"
    base.mkdir(parents=True, exist_ok=True)
    return base


class BoothApp(QApplication):
    def __init__(self, argv: list[str]):
        super().__init__(argv)
        self.setOrganizationName("BoothOrganizer")
        self.setApplicationName("BoothOrganizer")
        self.setApplicationVersion("0.1.0")

        self.app_data_dir = get_app_data_dir()
        self.db_path = self.app_data_dir / "booth.db"
