...
from booth_organizer.tools.registry import ToolConfig
from booth_organizer.unity_windows import bring_to_front

UNITY_JSON_PATH = Path(tempfile.gettempdir()) / "booth_thumb_latest.json"
...
class ToolLauncher(QObject):
    ...
    def launch_unity_fresh(self, filepath: Path, unity_exe: str,
        ...
        args = f"-projectPath \"{unity_project}\" -executeMethod BoothThumbnail.ProcessSingle"
        cmd = f"\"{unity_exe}\" {args}"
        try:
