...
from booth_organizer.ui.thumbnail_grid import (
    AssetListModel, ThumbnailDelegate, AssetListView, THUMB_SIZE,
)
from booth_organizer.ui.inspector import InspectorPanel
from booth_organizer.ui.tag_dialog import TagDialog
from booth_organizer.ui.context_menu import AssetContextMenu
from booth_organizer.ui.drop_overlay import DropOverlay
from booth_organizer.ui.sidebar import Sidebar
from booth_organizer.ui.settings_dialog import SettingsDialog
from booth_organizer.ui.cover_trainer import CoverTrainerDialog
from booth_organizer.tools.registry import ToolRegistry
from booth_organizer.tools.launcher import ToolLauncher
from booth_organizer.unity_windows import find_unity_editors, find_unity_project_path
from booth_organizer.workers.importer import ImportWorker
from booth_organizer.workers.thumb_worker import ThumbWorker
from booth_organizer.scanner.orchestrator import scan_file


class MainWindow(QMainWindow):
    def __init__(self, app: BoothApp, queries: Queries):
        super().__init__()
        self._app = app
        ...
        self.setWindowTitle("Booth Organizer")
            ...
            try:
                with zipfile.ZipFile(asset.filepath, "r") as zf:
                    for name in selected:
                        ...
                        import tempfile
                        tmp = Path(tempfile.gettempdir()) / "BoothOrganizer" / "extracted"
    ...
    def _on_open_with(self, tool_name: str, filepath: Path, asset_id: int = 0):
        ...
        if tool_name == "Unity Editor":
            ...
            if len(editors) == 0:
                ...
                self._status_label.setText(
                    f"Sent to {proj}. Switch to Unity and run Booth Thumbnail > Process Single."
                )
            else:
    ...
    def _show_unity_picker(self, editors: list[tuple[int, str]], filepath: Path,
        ...
        if item:
            ...
            self._tool_launcher.open_in_running_unity(hwnd, filepath, asset_tags, asset_filetype)
            self._status_label.setText(
                f"Sent to {item.text()}. Switch to Unity and run Booth Thumbnail > Process Single."
            )
    ...
    def _on_set_storage_path(self):
        ...
        if old_dir is None:
            ...
            default_dir = Path(os.environ.get("LOCALAPPDATA", "")) / "BoothOrganizer" / "Library"
    ...
    def _ask_migration_conflict(self, conflicts: dict[int, Path],
        ...
        from booth_organizer.models.asset import Asset
