...
from booth_organizer.auto_tagger import suggest_tags, suggest_genre
from booth_organizer.database.queries import Queries
from booth_organizer.scanner.orchestrator import scan_file
from booth_organizer.workers.base import BaseWorker
...
class ImportWorker(BaseWorker):
    ...
    def _import_one(self, filepath: Path) -> int | None:
        ...
        try:
            ...
            from booth_organizer.tag_data import GENRE_NAMES
    ...
    def _maybe_extract(self, filepath: Path) -> Optional[Path]:
        ...
        if not self._library_dir:
            self._library_dir = Path(os.environ.get("LOCALAPPDATA", "")) / "BoothOrganizer" / "Library"
