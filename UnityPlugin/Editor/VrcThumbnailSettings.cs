...
/// Open via Window > VRC Thumbnail Settings.
/// Settings persist to ProjectSettings/VrcThumbnail.json.
/// </summary>
public class VrcThumbnailSettingsWindow : EditorWindow
{
    private VrcThumbnail.VrcThumbnailSettings _settings;
    private Vector2 _scrollPos;

    [MenuItem("Window/VRC Thumbnail Settings")]
    public static void ShowWindow()
    {
        var win = GetWindow<VrcThumbnailSettingsWindow>("VRC Thumbnail");
        win.minSize = new Vector2(420, 320);
    }

    private void OnEnable()
    {
        _settings = VrcThumbnail.GetSettings();
    }
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.HelpBox(
            "When an asset is imported, its VRC Asset Organizer tags are matched against " +
        ...
        if (_settings.genreFolders == null)
            _settings.genreFolders = new VrcThumbnail.GenreEntry[0];
        ...
        for (int i = 0; i < _settings.genreFolders.Length; i++)
        {
            ...
            if (entry == null)
                entry = _settings.genreFolders[i] = new VrcThumbnail.GenreEntry();
        ...
        if (removeIndex >= 0)
        {
            var list = new List<VrcThumbnail.GenreEntry>(_settings.genreFolders);
        ...
        if (GUILayout.Button("+ Add Genre Mapping"))
        {
            var list = new List<VrcThumbnail.GenreEntry>(_settings.genreFolders)
            {
                new VrcThumbnail.GenreEntry { genre = "", folder = "Assets/" }
            };
            _settings.genreFolders = list.ToArray();
        }
        ...
        {
            VrcThumbnail.SaveSettings(_settings);
            EditorUtility.DisplayDialog("VRC Thumbnail", "Settings saved.", "OK");
        }
        ...
        if (GUILayout.Button("Reset to Defaults", GUILayout.Width(130), GUILayout.Height(28)))
        {
            if (EditorUtility.DisplayDialog(
                ...
                "Reset", "Cancel"))
            {
                _settings.genreFolders = new VrcThumbnail.GenreEntry[]
                {
                    new VrcThumbnail.GenreEntry { genre = "Avatar Base", folder = "Assets/1. Avatar Base" },
                    new VrcThumbnail.GenreEntry { genre = "Outfit & Acce", folder = "Assets/2. Outfit & Acce" },
                    new VrcThumbnail.GenreEntry { genre = "Gimmick", folder = "Assets/3. Gimmick" },
                    new VrcThumbnail.GenreEntry { genre = "Tools", folder = "Assets/4. Tools" },
                };
